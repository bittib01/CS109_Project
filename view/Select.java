...
public class Select extends JPanel {
    ...
    private void showLogin() {
        ...
        JPanel p = new JPanel(new GridLayout(2,2,5,5)); p.add(new JLabel("用户名:")); p.add(uf); p.add(new JLabel("密码:")); p.add(pf);
        int r=JOptionPane.showConfirmDialog(this,p,"用户登录",JOptionPane.OK_CANCEL_OPTION,JOptionPane.PLAIN_MESSAGE);
        if(r==JOptionPane.OK_OPTION){ String u=uf.getText().trim(),pw=new String(pf.getPassword());
            if(userController.login(u,pw)) refreshTopBar();
            else JOptionPane.showMessageDialog(this,"登录失败：用户名或密码错误","错误",JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void showRegister() {
        ...
        if(r==JOptionPane.OK_OPTION){ String u=uf.getText().trim(),pw=new String(pf.getPassword());
            if(userController.register(u,pw)){userController.login(u,pw);refreshTopBar();}
            else JOptionPane.showMessageDialog(this,"用户名已存在，注册失败！","错误",JOptionPane.ERROR_MESSAGE);
        }
    }

    // 预览面板
    private static class PreviewPanel extends BoardPanelBase {
        public PreviewPanel(){ super(null); setShowText(true); setText("请选择一个地图"); }
        public void setMap(GameMap map){ setShowText(false); this.board=new Board(map); repaint(); }
        public void setText(String t){ super.setText(t); repaint(); }
    }
    ...
    private static class InfoPanel extends JPanel {
        ...
        public InfoPanel(){ setLayout(new GridLayout(3,1)); add(sizeLabel); add(blockLabel); add(victoryLabel); }
        public void updateInfo(GameMap map){ sizeLabel.setText("尺寸："+map.getRows()+"×"+map.getCols()); blockLabel.setText("方块数："+map.getBlocks().size()); victoryLabel.setText("胜利区数："+map.getVictoryCells().size()); }
        public void updateInfo(String t){ sizeLabel.setText(t); blockLabel.setText(t); victoryLabel.setText(t); }
    }
    ...
    private static class StatsPanel extends JPanel {
        ...
        public StatsPanel(){ setLayout(new GridLayout(3,1)); add(completedLabel); add(movesLabel); add(timeLabel); }
        public void updateStats(String user, GameMap map){ Optional<Saver.Stats> opt=Saver.getStats(user,map);
            if(opt.isPresent()){ Saver.Stats s=opt.get(); completedLabel.setText("完成次数："+s.getCompletedCount()); if(s.getBestMoves()!=Integer.MAX_VALUE) movesLabel.setText("最少步数："+s.getBestMoves()); if(s.getBestTime()!=Long.MAX_VALUE) timeLabel.setText("最快时间："+formatTime(s.getBestTime())); }
            else clear(); }
        public void clear(){ completedLabel.setText("完成次数：-"); movesLabel.setText("最少步数：-"); timeLabel.setText("最快时间：-"); }
        private String formatTime(long ms){ long s=ms/1000, m=s/60; s%=60; return String.format("%d分%02d秒", m, s); }
    }
